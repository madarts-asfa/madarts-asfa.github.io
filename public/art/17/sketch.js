...
function setup() {
  ...
  instructions = createDiv(
    "Press SPACE to Start/Pause | Press F for Fullscreen"
  );
...
function updateParticles(vol) {
  for (let i = particles.length - 1; i >= 0; i--) {
    ...
    for (let angle = 0; angle < TWO_PI; angle += PI / 20) {
      let offset = map(
        noise(p.noiseSeed + angle, frameCount * 0.005),
        0,
        1,
        -10,
        12
...
function drawRadialGradient() {
  ...
  for (let r = radius; r > 0; r -= 5) {
    ...
    fill(240, alpha);
    noStroke();
    ellipse(width / 30, height / 30, r, r); //af fect of the swirl, width /30, height /30 || width /2*4, height /2*4,width /4, height /4
  }
}
...
function drawAbsorbingEllipse() {
  ...
  for (let i = 0; i < layers; i++) {
    ...
    for (let angle = 0; angle < TWO_PI; angle += PI / 20) {
      ...
      let offset =
        map(noise(angle, frameCount * 0.01), 0, 1, -20, 20) * noiseFactor;
...
function updateMaxSize() {
  maxEllipseSize = max(windowWidth, windowHeight) * 1.2;
}
