...
function setup() {
  ...
  for (let i = 0; i < numTiles; i++) {
    ...
    console.log("Tile " + (i + 1) + " : image " + img.myIndex +
                " | Energy Category: " + energyCat);
...
function draw() {
  ...
  for (let r of regions) {
    ...
    if (debug) {
      console.log("Tile " + regions.indexOf(r) + " energy cat:" + r.energyCategory +
                  " energy:" + energy + " activate? " + String(toActivate));
    }
  }
  ...
  update() {
    if (this.active) {
      if (millis() - this.lastStampTime > 200) {
        ...
        this.buffer.image(stamp, xPos, yPos, stamp.width * this.scalar, stamp.height * this.scalar);
        stamp.remove();
      }
    }
  }
...
function mousePressed() {
  ...
  let controlX = 10;
  let controlY = height - controlH - 10;
  if (mouseX >= controlX && mouseX <= controlX + controlW &&
...
function resetSketch() {
  ...
  for (let i = 0; i < numTiles; i++) {
    ...
    let energyCat = random(energyCategories);
    console.log("RESET: Tile " + (i + 1) + " : image " + img.myIndex +
                " | Energy Category: " + energyCat);
    let r = new Region(x, y, regW, regH, img);
    r.energyCategory = energyCat;
    r.activationTime = random(0, 10000);
